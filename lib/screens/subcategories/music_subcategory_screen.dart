...
class MusicSubcategoryScreen extends StatefulWidget {
  const MusicSubcategoryScreen({super.key});

  @override
  State<MusicSubcategoryScreen> createState() =>
      _MusicSubcategoryScreenState();
}

class _MusicSubcategoryScreenState
    extends State<MusicSubcategoryScreen> {
  ...
  Future<void> _loadPlayedQuestionCounts() async {
    ...
    for (final _MusicSubcategory item in _items) {
  ...
  Future<void> _loadFirebaseSubcategoryAvailability() async {
    try {
      final List<Future<QuerySnapshot<Map<String, dynamic>>>> checks =
          _items.map((_MusicSubcategory item) {
