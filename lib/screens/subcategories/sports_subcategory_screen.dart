...
class SportsSubcategoryScreen extends StatefulWidget {
  const SportsSubcategoryScreen({super.key});

  @override
  State<SportsSubcategoryScreen> createState() =>
      _SportsSubcategoryScreenState();
}

class _SportsSubcategoryScreenState
    extends State<SportsSubcategoryScreen> {
  ...
  Future<void> _loadPlayedQuestionCounts() async {
    ...
    for (final _SportsSubcategory item in _items) {
  ...
  Future<void> _loadFirebaseSubcategoryAvailability() async {
    try {
      final List<Future<QuerySnapshot<Map<String, dynamic>>>> checks =
          _items.map((_SportsSubcategory item) {
