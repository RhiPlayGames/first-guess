  ...
  Future<void> _loadFirebaseSubcategoryAvailability() async {
    try {
      final List<Future<QuerySnapshot<Map<String, dynamic>>>> checks =
          _items.map((_FamousWordsSubcategory item) {
        return FirebaseFirestore.instance
            .collection('challenges')
            .where('category', isEqualTo: 'famous_words')
      ...
      for (int i = 0; i < _items.length; i++) {
        ...
        final Set<String> ids = results[i].docs.map((doc) => doc.id).toSet();
            ...
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  final _FamousWordsSubcategory item = _items[index];
