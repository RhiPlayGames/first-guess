...
TARGET_SUBCATEGORY = "jungle_safari_animals"

OLD_DOCUMENT_IDS = [
    f"animals_jungle_safari_animals_{i:04d}"
    for i in range(1, 21)
]
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Safely remove only the 20 old Animals -> Jungle & Safari "
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="Actually delete the verified 20 old Firestore documents.",
    )
    args = parser.parse_args()

    print("FIRST GUESS — REMOVE OLD JUNGLE & SAFARI FIRESTORE QUESTIONS")
    print("=" * 72)
    ...
    if len(verified) != 20:
        stop(f"Safety check failed: expected 20 verified documents, found {len(verified)}.")
    ...
    if not args.write:
        ...
        print("If the 20 verified documents above are correct, run:")
        print("    python delete_old_jungle_safari_firestore.py --write")
        return
